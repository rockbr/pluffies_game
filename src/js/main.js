...
const RANKING_STORAGE_KEY = "plush-claw-ranking-v2";
const LEGACY_RANKING_STORAGE_KEYS = ["plush-claw-ranking-v1"];
...
function syncEmbeddedRankingUI() {
  ...
  if (rankingOpenAllButton) {
    rankingOpenAllButton.hidden = true;
    rankingOpenAllButton.setAttribute("aria-hidden", "true");
  }
...
function getPersistedSnapshot() {
  ...
  return {
    state: {
      ...
      carriedPlushes: state.carriedPlushes,
...
function normalizeRanking(entries) {
  ...
  entries.forEach((entry) => {
    ...
    const normalizedEntry = {
      ...
      caughtCount: Number(entry.caughtCount ?? entry.collection?.length ?? 0) || 0,
    };
    ...
    if (
      !current ||
      normalizedEntry.points > current.points ||
      (normalizedEntry.points === current.points && normalizedEntry.caughtCount > current.caughtCount)
    ) {
  ...
  return [...bestByName.values()]
    .sort((a, b) => (
      b.points - a.points ||
      b.caughtCount - a.caughtCount ||
      a.durationSec - b.durationSec ||
      a.name.localeCompare(b.name, "pt-BR")
    ));
}

function decorateRankingSummary() {
  if (rankingCountEl) {
...
function renderFullRankingModal() {
  ...
  rankingDetailTitleEl.textContent = "Ranking completo";
  rankingDetailMetaEl.textContent = `${rankingEntries.length} jogador${rankingEntries.length === 1 ? "" : "es"} registrados`;
  ...
  if (!rankingEntries.length) {
    ...
    rankingEntries.forEach((entry, index) => {
      ...
      row.className = "ranking-item ranking-item-compact";
      if (index < 3) {
        row.classList.add(`podium-${index + 1}`);
      }
      if (state.playerName && entry.name === state.playerName) {
      ...
      detail.textContent = `F${entry.phase} - ${entry.timeLabel} - ${entry.caughtCount} urso${entry.caughtCount === 1 ? "" : "s"}`;
      ...
      row.addEventListener("click", () => openRankingDetail(entry, groupCollectionForDisplay(entry.collection ?? [])));
      ...
      rankingDetailListEl.appendChild(row);
    });
  }
...
function registerRankingEntry() {
  ...
  rankingEntries.push({
    ...
    caughtCount: state.collection.length,
...
rankingOpenAllButton.addEventListener("click", renderFullRankingModal);
