...
export function drawGame(state, claw) {
  ...
  if (state.plushVisible !== false && claw.carrying) {
    (state.carriedPlushes?.length ? state.carriedPlushes : [state.plush].filter(Boolean)).forEach((plush) => {
      drawPlush(plush);
    });
  }
  ...
  if (state.exitAnimation) {
    (state.exitAnimations?.length ? state.exitAnimations : [state.exitAnimation]).forEach((animation) => {
      drawExitAnimation(animation);
    });
  }
