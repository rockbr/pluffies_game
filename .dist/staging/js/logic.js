...
function triggerGameOver(state, claw, message) {
  ...
  setMessage(state.continuePrompt
    ? `Continue disponível. Você tem ${Math.ceil(state.continueTimer / 1000)}s para usar.`
    : message);
}
...
function getCatchCandidates(state, claw) {
  const phase = phases[state.phaseIndex];
  ...
  candidates.forEach((plush) => {
    ...
    if (alignmentScore >= phase.catchThreshold) {
      caughtTargets.push({ plush, alignmentScore, horizontalDistance });
    }
  });

  return caughtTargets
    .sort((a, b) => (
      b.alignmentScore - a.alignmentScore ||
      a.horizontalDistance - b.horizontalDistance ||
      a.plush.x - b.plush.x
    ))
    .map((entry) => entry.plush);
}
...
function handleWin(state, claw) {
  ...
  state.exitAnimations = caughtPlushes.map((plush) => ({
    progress: 0,
    startX: plush.x,
    startY: plush.y,
  ...
  state.exitAnimation = state.exitAnimations[0] ?? null;
  ...
  caughtPlushes
    ...
    .reverse()
    .forEach((plush) => {
      const plushPoints = phase.points * getPointMultiplier(plush.variant);
      state.collection.unshift({
        name: getPrizeName(plush, state.collection.length + 1),
        detail: `${phase.name} - ${plushPoints} pontos`,
        color: plush.color,
        accent: plush.accent,
        variant: plush.variant,
        themeId: plush.themeId ?? "",
  ...
  state.phaseCatchCount += caughtPlushes.length;
  state.bigBearMashRemaining = 0;
  state.carriedPlushes = [];
...
function handleSkullCatch(state, claw, skullPlush) {
  ...
  setMessage(state.bonusRound
    ? `Urso caveira pego. Você perdeu ${penalty} pontos e a rodada x${state.bonusRound.multiplier} foi perdida.`
    : `Urso caveira pego. Você perdeu ${penalty} pontos.`);
}
...
function resolveDropAtBottom(state, claw) {
  ...
  const caughtPlushes = getCatchCandidates(state, claw);
  const skullPlush = caughtPlushes.find((plush) => plush.variant === "skull");

  if (skullPlush) {
    handleSkullCatch(state, claw, skullPlush);
    return;
  }

  if (caughtPlushes.length) {
    const leadPlush = caughtPlushes[0];
    const totalPhasePoints = caughtPlushes.reduce(
      (total, plush) => total + phase.points * getPointMultiplier(plush.variant),
      0,
    );
    const rarityLabel = VARIANT_RARITY_LABELS[leadPlush.variant];
    claw.carrying = true;
    claw.dropping = false;
    claw.returning = true;
    state.plushVisible = true;
    state.plush = leadPlush;
    state.carriedPlushes = caughtPlushes.map((plush) => ({ ...plush }));
    state.plushes = (state.plushes ?? []).filter((plush) => !caughtPlushes.includes(plush));
    if (caughtPlushes.includes(state.specialPlush)) {
      state.specialPlush = null;
    }
    state.bigBearMashRemaining = caughtPlushes.reduce(
      (total, plush) => total + (plush.variant === "big" ? 2 : 0),
      Math.max(0, caughtPlushes.length - 1),
    );
    ...
    triggerVisualFeedback(state, {
      ...
      y: leadPlush.y,
      color: "#6ae4a0",
      label: caughtPlushes.length > 1
        ? `${caughtPlushes.length}x +${totalPhasePoints}`
        : (rarityLabel ? `${rarityLabel} +${totalPhasePoints}` : `+${totalPhasePoints}`),
      pulseTimer: 420,
      overlayTimer: 180,
      shake: 6,
    });
    setMessage(
      state.bigBearMashRemaining > 0
        ? `${caughtPlushes.length} urso(s) pegos. Aperte PEGAR mais ${state.bigBearMashRemaining} vez(es) para subir.`
...
export function beginDrop(state, claw) {
  if (claw.carrying && state.bigBearMashRemaining > 0) {
    ...
    setMessage(
      state.bigBearMashRemaining > 0
        ? `Carga pesada. Aperte PEGAR mais ${state.bigBearMashRemaining} vez(es).`
        : "Carga liberada. Subindo!"
    );
    return;
  }
...
export function updateGame(state, claw, deltaMs) {
  ...
  if (claw.dropping) {
    ...
    if (claw.carrying && state.bigBearMashRemaining > 0 && claw.armHeight <= 98) {
      claw.armHeight = 98;
    } else {
    claw.armHeight -= 10 * slowFactor * deltaFactor;
    if (claw.armHeight <= 0) {
  ...
  if (claw.carrying) {
    ...
    carriedPlushes.forEach((plush, index) => {
      const spreadX = (index - (carriedPlushes.length - 1) / 2) * 18;
      plush.x = claw.x + spreadX;
      plush.y = claw.y + claw.armHeight + 114 + index * 8;
    });
    state.plush = carriedPlushes[0] ?? null;
  ...
  if (state.exitAnimation) {
    state.exitAnimations = (state.exitAnimations?.length ? state.exitAnimations : [state.exitAnimation])
      .filter(Boolean)
      .map((animation) => ({ ...animation, progress: animation.progress + deltaMs / 560 }));
    state.exitAnimation = state.exitAnimations[0] ?? null;
    if (state.exitAnimations.every((animation) => animation.progress >= 1)) {
      state.exitAnimation = null;
      state.exitAnimations = [];
      if (state.phaseCatchCount >= targetCaptures) {
